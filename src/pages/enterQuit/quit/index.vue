...
// 接口
import { getSelectByPage } from '@/api/synergy'
...
const getList = async () => {
  ...
  try {
    const res: any = await getSelectByPage(pagination.value) // 获取列表数据
...
const handleClear = (v) => {
  if (v === 'retreatCode') {
    pagination.value.retreatCode = ''
  } else if (v === 'name') {
    pagination.value.name = ''
  } else if (v === 'idCardNo') {
    pagination.value.idCardNo = ''
  } else {
